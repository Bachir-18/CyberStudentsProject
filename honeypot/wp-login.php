...
$log_file = '/var/log/honeypot/sqli_attempts.log';

// SQL Injection Detection Patterns
$sqli_patterns = [
    '/(union\s+select|select\s+\w+\s+from)/i',   // SQL SELECT statements
    '/sleep\(\d+\)/i',                          // Time-based SQLi
    '/benchmark\(\d+/i',                        // Benchmark attacks
    '/\'\s*or\s*1=1/i',                         // Logical SQLi
    '/;[\s-]*\w+/',                             // SQL statement chaining
    '/\b(exec|declare|cast|insert|update)\b/i', // Common SQL commands
];
...
foreach (array_merge($_GET, $_POST) as $param => $value) {
    foreach ($sqli_patterns as $pattern) {
        if (is_string($value) && preg_match($pattern, $value)) {
            // Log the attempt
            $log_entry = sprintf(
                "[%s] IP: %s | User-Agent: %s | Param: %s=%s\n",
                date('Y-m-d H:i:s'),
                $_SERVER['REMOTE_ADDR'],
                $_SERVER['HTTP_USER_AGENT'] ?? 'No-UA',
                $param,
                base64_encode(substr($value, 0, 500)) // Encode payload for safety
            );
            ...
            $error_message = match (rand(1, 6)) {
                1 => sprintf("Syntax error near '%s'", htmlspecialchars(substr($value, 0, 20))),
                2 => "Could not connect to database server",
                3 => "Query execution time exceeded",
                4 => "Invalid character in expression",
                5 => "Type mismatch in comparison",
                default => "Database operation failed",
            };

            // Delay to simulate server load
            sleep(rand(2, 5));
...
function render_honeypot_login_form($error_message = '') {
    ?>
    <form name="loginform" id="loginform" action="" method="post">
        ...
        <?php if (!empty($error_message)) : ?>
            <div style="color: red; font-family: monospace; margin-top: 10px;">
...
// Show the form with an error message if SQLi was detected
if ($sqli_detected) {
    render_honeypot_login_form($error_message);
    exit;
}
